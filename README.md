...
- 🌓 **Fluent Design 设置面板** — 暗色/亮色主题切换，图形化选角选装界面。支持简中、繁中、英文、日文、葡萄牙语、西班牙语、法语、德语、俄语 i18n 国际化。
- 📌 **始终置顶 + 无边框** — 趴在你的窗口上方，赶都赶不走（误）。
- 🔔 **系统托盘** — 右键一键切角色 / 开设置 / 优雅退场。
- 🔊 **TTS 语音合成** — 接入 Qwen3TTS 后端，角色说话时自动语音播放 + Live2D 口型同步，支持中日英多语言。
- 🎙️ **ASR 语音输入** — 支持一键安装本地 faster-whisper 后端，也可接入 OpenAI-compatible 语音识别服务，把麦克风录音转成聊天输入文本，详见 [ASR_VOICE_INPUT_GUIDE.md](ASR_VOICE_INPUT_GUIDE.md)。
...
| **Ave Mujica** | 丰川祥子 · 若叶睦 · 三角初华 · 八幡海铃 · 祐天寺若麦 | ✅ | ✅ |
| **其他角色** | 纯田真奈 · 户山明日香 等 | ✅ | ✅ |
...
原生应用使用 CMake + CPack 构建，不需要 Python：
...
需要 Qt 6.5+（Core、Gui、Widgets、Multimedia、Svg、OpenGLWidgets、WebSockets）、
Rust 1.85+、CMake 3.24+ 与 C++17 工具链。安装包输出在 `build-rust/`，Windows
生成 ZIP/NSIS，macOS 生成 DMG，Linux 生成 TGZ/DEB。默认不内置模型；如需制作
明确包含仓库模型的发行包，在配置阶段增加
`-DBANDORI_PET_PACKAGE_BUNDLED_MODELS=ON`。

发布新版本时修改根目录 [`VERSION`](VERSION)。兼容回退窗口内若仍发布旧版
Python 构建，再同步修改 `app_info.py`；它不再参与原生 CMake 构建。
三平台 CI、签名密钥名称、macOS notarization 和标签发布步骤见
[`docs/native-release.md`](docs/native-release.md)。

CPack 资产名采用以下形式：

```text
BandoriPet-3.1.4-Windows-AMD64.zip
BandoriPet-3.1.4-Windows-AMD64.exe
BandoriPet-3.1.4-Darwin-arm64.dmg
BandoriPet-3.1.4-Linux-x86_64.tar.gz
BandoriPet-3.1.4-Linux-x86_64.deb
```
