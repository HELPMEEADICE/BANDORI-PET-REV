...
macOS 的 PyPI `lupa` wheel 不包含 Live2D 所需的 `lupa.luajit21`。完成上述第三方
依赖安装并激活虚拟环境后，使用专用脚本安装常规依赖、编译 LuaJIT 2.1，并为
Live2D-v2-Lua 应用图片字节流补丁：
...
> 💡 **语音克隆**：`audio_reference/` 目录下已内置 61 位角色的日文参考音频，TTS 会自动根据当前聊天角色选择对应的声线。
...
已收录 12 个乐团及其他角色；安装相应 Live2D 模型后可显示角色与服装。已配置 LLM 角色扮演 Prompt 的角色可开启 AI 对话。
...
| **梦限大MewType** | 仲町阿拉蕾 · 宫永野乃花 · 峰月律 · 藤都子 · 千石由乃 | ✅ | ✅ |
| **millsage** | 汐见萤 · 伊泽枣 · 琴平凪 · 滨崎茉幌 · 和泉朋花 | ✅ | ✅ |
| **一家Dumb Rock!** | 须贺蕾叶 · 马桥心玖 · 矢仓蓬咲 · 梅里千樱梨 · 四宫宁月 | ✅ | ✅ |
| **其他角色** | 纯田真奈 · 户山明日香 等 | ✅ | ✅ |

> 角色已配置本地角色扮演 Prompt（`characters/` 目录）。更多角色的 Prompt 模板见 `PROMPT.md`，欢迎提交 PR 补全！
