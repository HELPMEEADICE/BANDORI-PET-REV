...
def ipc_reliable_inbound_queue_key() -> str:
    return make_shared_memory_key(ipc_server_name(), "main-reliable-in")
...
def is_reliable_ipc_line(line: str) -> bool:
    normalized = normalize_ipc_line(line)
...
def _send_ipc_message_locked(message: str) -> bool:
    global _ipc_inbound_queue, _ipc_reliable_inbound_queue
    try:
        ...
        if not lines:
            return False
        for line in lines:
            if is_reliable_ipc_line(line):
                if _ipc_reliable_inbound_queue is None or not _ipc_reliable_inbound_queue.is_attached():
                    _ipc_reliable_inbound_queue = SharedMemoryLineQueue.attach(
                        ipc_reliable_inbound_queue_key()
                    )
                if not _ipc_reliable_inbound_queue.publish(
                    encode_ipc_envelope(_ipc_sender_id, line, reliable=True)
                ):
                    return False
            else:
                ...
                if not _ipc_inbound_queue.publish(encode_ipc_envelope(_ipc_sender_id, line)):
                    return False
        return True
    except Exception:
        if _ipc_inbound_queue is not None:
            ...
            _ipc_inbound_queue = None
        if _ipc_reliable_inbound_queue is not None:
            _ipc_reliable_inbound_queue.close()
            _ipc_reliable_inbound_queue = None
        return False
...
def attach_main_ipc_queues(ipc: dict) -> bool:
    try:
        ...
        if ipc.get("reliable_inbound") is None or not ipc["reliable_inbound"].is_attached():
            ipc["reliable_inbound"] = SharedMemoryLineQueue.attach(
                ipc_reliable_inbound_queue_key()
            )
        ...
        for key in ("inbound", "reliable_inbound", "broadcast", "control"):
            queue = ipc.get(key)
